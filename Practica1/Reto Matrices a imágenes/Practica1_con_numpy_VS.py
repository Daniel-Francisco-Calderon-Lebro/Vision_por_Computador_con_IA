...
def programa_con_numpy():
    ...
    def crear_imagen_de_0_255(filas, columnas):
        imagen=np.random.randint(0, 255, (filas, columnas))
        return imagen
    ...
    def datos_estadisticos(imagen):
        return np.amin(imagen), np.amax(imagen), np.mean(imagen), np.std(imagen)
    ...
    def generador_archivos(imagenaplanada):
        ...
        with open("imagenaplanadaconnumpy.txt", "w") as archivo:
        ...
        with open("imagenaplanadaconnumpy.csv", "w") as archivo:
            for pixel in imagenaplanada:
                # Escribir el pixel separado por comas
                if pixel != imagenaplanada[-1]:
                    archivo.write(str(pixel) + ",")
        ...
        with open("imagenaplanadaconnumpy.tsv", "w") as archivo:
            for pixel in imagenaplanada:
                # Escribir el pixel separado por comas
                if pixel != imagenaplanada[-1]:
                    archivo.write(str(pixel) + "\t")
        ...
        with open("imagenaplanadaconnumpy.out", "w") as archivo:
            for pixel in imagenaplanada:
                # Escribir el pixel separado por comas
                if pixel != imagenaplanada[-1]:
                    archivo.write(str(pixel) + " ")
